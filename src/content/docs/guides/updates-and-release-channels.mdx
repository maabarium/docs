...
Beta releases are intentionally manual so they do not replace the stable install path:

1. create a prerelease or a tagged GitHub Release for the build you want to test
2. run the desktop release workflow manually with that release tag and `release_channel=beta`
3. the workflow publishes `beta/latest.json` without replacing the root `latest.json` stable alias
